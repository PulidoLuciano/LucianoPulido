...
public class CommentServiceImpl extends GenericServiceImpl<Comment, UUID, CommentRepository> implements CommentService{
    ...
    public Set<Comment> getResponsesById(UUID id) {
        Comment parent = super.getRepositorio().findById(id).orElseThrow(() -> new IllegalArgumentException("The comment does not exist"));
        return parent.getResponses();
    }

    @Override
    public Comment createResponseByParentId(UUID id, String message, String token) {
        Comment comment = super.getRepositorio().findById(id).orElseThrow(() -> new IllegalArgumentException("The comment does not exist"));
        ...
        super.getRepositorio().save(comment);
        return response;
    }
